...
class KelebekSubWindow(NodeEditorWidget):
    ...
    def doEvalOutputs(self):
        ...
        for node in self.scene.nodes:
            if node.__class__.__name__ == "KelebekNode_Output":
                node.eval()
    ...
    # helper functions
    def determine_target_socket_of_node(self, was_dragged_flag, new_kelebek_node):
        target_socket = None
        if was_dragged_flag:
            if len(new_kelebek_node.inputs) > 0: target_socket = new_kelebek_node.inputs[0]
        else:
            if len(new_kelebek_node.outputs) > 0: target_socket = new_kelebek_node.outputs[0]
        return target_socket

    def finish_new_node_state(self, new_kelebek_node):
        self.scene.doDeselectItems()
        new_kelebek_node.grNode.doSelect(True)
        new_kelebek_node.grNode.onSelected()
    ...
    def handleNewNodeContextMenu(self, event):
        ...
        if action is not None:
            ...
            if self.scene.getView().mode == MODE_EDGE_DRAG:
                ...
                target_socket = self.determine_target_socket_of_node(self.scene.getView().dragging.drag_start_socket.is_output, new_kelebek_node)
                if target_socket is not None:
                    self.scene.getView().dragging.edgeDragEnd(target_socket.grSocket)
                    self.finish_new_node_state(new_kelebek_node)
