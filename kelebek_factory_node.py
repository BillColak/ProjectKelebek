...
class ResizableNode(QGraphicsObject):
    ...
    def move_sockets(self, s):
        for output_socket in self.node.outputs:
            ...
            y = top_offset + available_height / 2.0 + (output_socket.index-0.5)*22
        ...
        for input_socket in self.node.inputs:
            ...
            if input_socket.position in (LEFT_BOTTOM, RIGHT_BOTTOM):
                y = s.height() - self.edge_roundness - self.title_vertical_padding - input_socket.index * 22
                ...
                y = top_offset + available_height / 2.0 + (input_socket.index - 0.5) * 22
                if num_sockets > 1:
                    y -= 22 * (num_sockets - 1) / 2
            elif input_socket.position in (LEFT_TOP, RIGHT_TOP):

                y = self.title_height + self.title_vertical_padding + self.edge_roundness + input_socket.index * 22
            else:
                y = 0
